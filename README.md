...
The `--exec-profile` flag allows for execution with an alternate aws profile (via ~/.aws/config) 
...
Example (which assumes different role)

~/.aws/credentials: (these are generated by `saml2aws login`)
```
[saml]
...
saml2aws exec aws sts get-caller-identity
{
    ...
    "Arn": "arn:aws:sts::000000000123:assumed-role/myInitialAccount"
}
...
```
saml2aws exec --exec-profile otherRole aws sts get-caller-identity
{
