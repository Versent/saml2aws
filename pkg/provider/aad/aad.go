...
func (ac *Client) Authenticate(loginDetails *creds.LoginDetails) (string, error) {
	...
	// startSAML
	startURL := fmt.Sprintf("https://account.activedirectory.windowsazure.com/applications/redirecttofederatedapplication.aspx?Operation=LinkedSignIn&applicationId=%s", ac.idpAccount.AppID)
	...
	for scanner.Scan() {
		...
		if strings.Contains(scanLine, "$Config") {
			startSAMLJson = strings.TrimSuffix(strings.TrimPrefix(scanLine, "$Config="), ";")
			break
		}
	}
	...
	for scanner.Scan() {
		scanLine := strings.TrimSpace(scanner.Text())
		if strings.Contains(scanLine, "$Config") {
			loginPasswordJson = strings.TrimSuffix(strings.TrimPrefix(scanLine, "$Config="), ";")
			break
		}
	}
	...
	for scanner.Scan() {
		scanLine := strings.TrimSpace(scanner.Text())
		if strings.Contains(scanLine, "$Config") {
			ProcessAuthJson = strings.TrimSuffix(strings.TrimPrefix(scanLine, "$Config="), ";")
			break
		}
	}
	...
	// kmsi
	// url =  res.Request.URL.Scheme + "://" + res.Request.URL.Host + processAuthResp.URLPost
